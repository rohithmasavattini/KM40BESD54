...
void get_date_time(void)
{
	...
	printf("\n-----AVILABEL RTC DEVICES-----\n");
        system("ls  /dev/ | grep  rtc | grep -vw rtc");
        printf("\n------------------------------\n");
        printf("Pls Enter rtc Number 0 or 1 \n");
        scanf(" %d",&rtc_number);
        sprintf(input_buff,"/dev/rtc%d",rtc_number);
	/* Creating a file descriptor for RTC */
	fd = open(input_buff, O_RDONLY);
	if (fd == -1) {
		printf("/dev/rtc%d not avilable \n",rtc_number);
